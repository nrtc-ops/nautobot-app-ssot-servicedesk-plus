...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_SSOT_SERVICEDESK_PLUS_xxx
namespace = Collection("nautobot_ssot_servicedesk_plus")
namespace.configure(
    {
        "nautobot_ssot_servicedesk_plus": {
            "nautobot_ver": "3.0.0",
            "project_name": "nautobot-ssot-servicedesk-plus",
            "python_ver": "3.12",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_ssot_servicedesk_plus.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_ssot_servicedesk_plus.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_ssot_servicedesk_plus.nautobot_ver,
        "PYTHON_VER": context.nautobot_ssot_servicedesk_plus.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_ssot_servicedesk_plus.project_name}",
        f'--project-directory "{context.nautobot_ssot_servicedesk_plus.compose_dir}"',
    ]

    for compose_file in context.nautobot_ssot_servicedesk_plus.compose_files:
        compose_file_path = os.path.join(context.nautobot_ssot_servicedesk_plus.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_ssot_servicedesk_plus.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_ssot_servicedesk_plus.python_ver}...")
    docker_compose(context, command)


def _ensure_creds_env_file(context):
    """Ensure that the development/creds.env file exists."""
    if not os.path.exists(os.path.join(context.nautobot_ssot_servicedesk_plus.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_ssot_servicedesk_plus.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_ssot_servicedesk_plus.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_ssot_servicedesk_plus.python_ver
    dockerfile_path = os.path.join(context.nautobot_ssot_servicedesk_plus.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_ssot_servicedesk_plus.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_ssot_servicedesk_plus"
...
@task(
    help={
        "version": "Version of Nautobot SSOT ServiceDesk Plus to generate the release notes for.",
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_ssot_servicedesk_plus"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_ssot_servicedesk_plus") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " nautobot_ssot_servicedesk_plus.migrations"
        )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_ssot_servicedesk_plus/templates/"
...
def unittest(  # noqa: PLR0913
    context,
    keepdb=False,
    label="nautobot_ssot_servicedesk_plus",
    failfast=False,
    buffer=True,
    pattern="",
    verbose=False,
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_ssot_servicedesk_plus.local):
